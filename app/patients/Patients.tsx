...
interface pageProps {
    ...
    doctor_list: Array<clinic_users>
}
export default function Patients({ patient_list, doctor_list }: pageProps) {

    const [loading, setLoading] = useState(true);
    const [curPatient, setCurPatient] = useState({} as patient_record);
    const [curAttachments, setCurAttachments] = useState([] as image_attachments[])

    const getPatient = async (patient_id: String) => {
        return await fetch(`/api/patient-summary/?patient_id=${patient_id}`).then((res) => res.json())
    }

    const getImages = async (patient_id: String) => {
        return await fetch(`/api/patient-attachments/?patient_id=${patient_id}`).then((res) => res.json())
    }

    const onClick = async (patient_id: String) => {
        setLoading(true);
        const notes = await getPatient(patient_id);
        const attch = await getImages(patient_id);
        setCurPatient(notes.responseBody);
        setCurAttachments(attch.responseBody);
        setLoading(false);
    }
    ...
    useEffect(() => {
        try {
            const init = async () => {
                ...
                const notes = await getPatient(String(patient_list[0].patient_id));
                const attch = await getImages(String(patient_list[0].patient_id));
                setCurPatient(notes.responseBody);
                setCurAttachments(attch.responseBody);
                setLoading(false);
            }
    ...
    return (
        <div className="w-screen flex">
            <div className="container w-screen h-screen flex gap-5 justify-center mt-[5rem] p-5">
                <section className="relative shadow-xl rounded-lg w-1/4 h-[35rem] p-4 bg-[#F2F1EB]">
                    <div className="overflow-scroll h-full">
                        <p className="bg-[#F2F1EB] flex gap-2 text-xl font-semibold">
                                        ...
                                        <div key={idx} className="p-2">
                                            <div className="flex gap-4 items-center justify-between">
                                                <div className="flex gap-4 items-center">
                                                    <img className="w-8 h-8 rounded-full p-1" src="https://i.kym-cdn.com/entries/icons/facebook/000/046/895/huh_cat.jpg" />
                ...
                <section className="rounded-lg shadow-xl bg-[#F2F1EB]  w-1/2 h-[35rem] p-2 flex">
                    <div className="overflow-scroll flex justify-center items-center h-full w-full">
                        {
                            loading ? <svg aria-hidden="true" className="w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <PatientSummary curPatient={curPatient} images={curAttachments} doclist={doctor_list} />
                            </div>
                        }
                    </div>
                </section>
